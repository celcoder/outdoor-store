...
var random = function(allUsers) {
  return Math.floor(Math.random() * (allUsers.length));
}
...
var seedOrders = () => {
  ...
  var creatingOrders = orders.map(orderObj => { //iterate through the orders array;
    var order;
    return Order.create(orderObj) //create orders table.
      .then(newOrder => {
        order = newOrder; //assign newOrder to variable order.
        return User.findAll({}); //find all Users.
      })
      .then(allUsers => {
        return order.setUser(allUsers[random(allUsers)]); //make the order association with random users.
      })
      .then(userOrder => {
        order = userOrder; //assign the userOrder to variable.
        return Product.findAll({}); //find all products
      })
      .then(allProducts => {
        var randomProducts = _.sample(allProducts, 4); //lodash method _.sample
        return order.addProducts(randomProducts); //make the userOrder association with products.
      })
  });
